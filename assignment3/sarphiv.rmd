...
x_reg_diff <- apply(diffinv(data.matrix(x_reg), 4)[-(1:4), ], 2, cumsum)
...
unix_model <- Arima(
  ...
  lambda = 0,
  biasadj = T,
  xreg = head(x_reg_diff, n_uni_res)
)
...
pacf(cbind(diff(diff(log(data$Denmark), 4), 1), head(x_reg_diff, n_uni_res+1)), na.action = na.omit)
# TODO: Create a design matrix with different lags of the regressors
# TODO: Need to maybe do that for the MARIMA too
...
```{R}
unix_model_forecast <- forecast(unix_model, h = 6, level = c(95), xreg = tail(x_reg_diff, 6))
...
data_m <- cbind(
  "int1" = stats::lag(ts(x_reg[, 1]), 1),
  "int4" = stats::lag(ts(x_reg[, 1]), 4),
  "inf1" = stats::lag(ts(x_reg[, 2]), 1),
  "inf4" = stats::lag(ts(x_reg[, 2]), 4)
)
# data_m <- cbind(
#   "int1" = rnorm(131),
#   "int4" = rnorm(131),
...
#   "inf4" = rnorm(131)
# )
data_m[is.na(data_m[, 2]), 2] = data$InterestRate[124]
data_m[is.na(data_m[, 4]), 4] = data$InflationRate[124]
data_m <- t(cbind(
  log(subset(data, select = c(Capital, Sealand, MidJutland, Rural))), 
  data_m[4:nrow(data_m), ]
))
...
for (i in 1:4) {
  ...
  mulx_pred[i, , ] <- exp(matrix(rep(mulx_forecast$forecasts[i, (ncol(data_mp) - 5):ncol(data_mp)], 3), nrow = 6) + cbind(rep(0, 6), -1, 1) * qnorm(0.975) * sqrt(mulx_forecast$pred.var[i, i, ]))
...
for (i in 1:4) {
  ...
  matlines((ncol(data_mp) - 5):ncol(data_mp), mulx_pred[i, 1:6, ], lty = c(1, 2, 2), col = mulx_colors[i], lwd = 1)
