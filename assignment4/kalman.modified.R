kalman.modified <-
  function(Y,
    ...
    if (verbose) {
      K.out <-
        array(dim = c(dim(Sigma.xx %*% t(C) %*% solve(Sigma.yy)), dim.Y[1]))
      Sigma.xx.rec <- array(dim = c(dim(Sigma.xx), dim.Y[1]))
      Sigma.yy.rec <- array(dim = c(dim(Sigma.yy), dim.Y[1]))
      Sigma.xx.pred <-
        array(dim = c(dim(Sigma.xx), dim.Y[1] + n.ahead))
      Sigma.yy.pred <-
        array(dim = c(dim(Sigma.yy), dim.Y[1] + n.ahead))
    ...
    for (tt in (skip + 1):dim.Y[1]) {
      ...
      if (!any(is.na(Y[tt, ]))) {
        # Check if inlier
        if (abs(t(Y[tt, ]) - C %*% as.matrix(X.hat)) < 6 * sqrt(Sigma.yy)) {
          # At first everything is thrown away if one is missing
      ...
      X.hat <-
        A %*% X.hat + B %*% t(matrix(as.numeric(u[tt, ]), nrow = 1))
      X.pred[tt + 1, ] <- X.hat
